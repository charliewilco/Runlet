...
async fn event_sequencing_is_monotonic() {
    ...
    store
        ...
        .unwrap();
    store
        .emit(&job_id, EventKind::Stdout, serde_json::json!({"line":"hello"}))
        .await
        .unwrap();
    store
        .emit(&job_id, EventKind::JobCompleted, serde_json::json!({"exit_code":0}))
        .await
        .unwrap();
...
async fn run_job_emits_events_and_completes() {
    ...
    let spec = JobSpec {
        cmd: "bash".to_string(),
        args: vec!["-lc".to_string(), "echo hello && sleep 0.2 && echo world".to_string()],
        cwd: None,
        env: None,
    };
