...
pub async fn sse_stream(
    ...
    let after_seq = last_event_id_from_headers(&headers_in).or(params.last_event_id).unwrap_or(0);
    let history = state
        .store
    ...
    let response = Sse::new(stream).keep_alive(KeepAlive::new().interval(std::time::Duration::from_secs(10)));
...
fn to_event(record: EventRecord) -> Result<Event, std::convert::Infallible> {
    let event = Event::default()
        ...
        .unwrap_or_else(|_| Event::default().event("job.failed").data("{\"message\":\"event encoding failed\"}"));
    Ok(event)
}
